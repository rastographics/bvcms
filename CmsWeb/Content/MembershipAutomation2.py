...
def DropMembership(p, Db):
    ...
    if p.SpouseId != None:
        spouse = CurrentDatabase.LoadPersonById(p.SpouseId)
...
class MembershipAutomation:
    def Run(self, Db, p):
        ...
        om2 = CurrentDatabase.LoadOrgMember(p.PeopleId, "Step 1", False)
        if om2 != None:
            om2 = None
		...
		and p.NewMemberClassStatusId == NewMemberClassStatusCode.Attended):
            om = CurrentDatabase.LoadOrgMember(p.PeopleId, "Step 1", False)
            if om != None:
                om.Drop(CurrentDatabase. True) # drops and records drop in history
