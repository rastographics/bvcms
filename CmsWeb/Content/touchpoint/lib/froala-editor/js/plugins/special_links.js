!function (a) {
    a.Editable.commands = a.extend(a.Editable.commands, {
        ...
        a.Editable.prototype.specialLinkHTML = function() {
            ...
            return b += '<div class="f-popup-line"><div><strong>Url</strong></div><input type="text" placeholder="http://www.example.com" class="f-lu" id="f-special-link-url-' + this._id + '"></div><div class="f-popup-line"><div><strong>OrgId / Meeting Id</strong></div><input type="text" class="f-lu" id="f-special-link-orgid-' + this._id + '"></div><div class="f-popup-line"><div><strong>Message</strong></div><input type="text" class="f-lu" id="f-special-link-message-' + this._id + '"></div><div class="f-popup-line"><div><strong>Small Group</strong></div><input type="text" class="f-lu" id="f-special-link-small-group-' + this._id + '"></div><div class="f-popup-line"><div><strong>Confirmation</strong></div><select id="f-special-link-confirmation-' + this._id + '" style="width: 250px; height: 25px; border: solid 1px #cccccc;"><option value="">&lt;not set&gt;</option><option value="ltr">Yes, send confirmation</option><option value="rtl">No, do not send confirmation</option></select></div><div class="f-popup-line"><input type="checkbox" id="f-special-link-new-tab-' + this._id + '"> <label data-text="true" for="f-special-link-new-tab-' + this._id + '">Open in new tab</label><button id="f-special-link-submit-' + this._id + '" type="button" data-text="true" class="fr-p-bttn f-ok f-submit">OK</button></div></div>';
        };
        ...
        a.Editable.prototype.buildSpecialLink = function() {
            var b = this;
            var url = b.$special_link_wrapper.find("#f-special-link-url-" + b._id);
            ...
            if (url.val().length > 0) {
                ...
                b.writeLink(url.val(), null, null, newTab.is(":checked"), true);
