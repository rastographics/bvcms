...
namespace CmsWeb.Areas.Public.Controllers
{
    public class MobileAPIv2Controller : CMSBaseController
    {
        ...
        public MobileMessage FetchGivingSummary(string data)
        {
            ...
            if (year == 0)
            {
                year = givingYears.Count > 0 ? givingYears.First() : DateTime.Now.Year;
            }
            var summary = new MobileGivingSummary(givingYears);
            var FromDate = new DateTime(year, 1, 1);
            var ToDate = new DateTime(year, 12, 31);
            var ci = new ContributorInfo
            {
                FamilyId = user.Person.FamilyId,
        ...
        public ActionResult DetailedGivingStatement(string data)
        {
            ...
            return new ContributionStatementResult(CurrentDatabase)
            {
                ...
                typ = user.Person.PositionInFamilyId == PositionInFamily.PrimaryAdult ? 2 : 1,
                noaddressok = true,
                useMinAmt = false,
