...
WHERE EXISTS(SELECT NULL FROM dbo.DivOrg WHERE OrgId = OrganizationId AND DivId = {0})", DivId);
        ...
        public void SendMovedNotices()
        {
            ...
            foreach (var i in q)
            {
                var subj = DbUtil.Db.ContentHtml("OrgMembersModel_SendMovedNotices_Subject", Resource1.OrgMembersModel_SendMovedNotices_Subject);

                if (subj == null)
                {
                    subj = $"{i.OrganizationName} room assignment";
                }
                else
                {
