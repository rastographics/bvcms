...
namespace CmsWeb.Areas.People.Models
{
    public class PersonModel
    {
        ...
        {
            var flags = DbUtil.CurrentDatabase.Setting("StatusFlags", "F04,F01,F02,F03");
            var isvalid = Regex.IsMatch(flags, @"\A(F\d\d,{0,1})(,F\d\d,{0,1})*\z", RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture);

            var i = (from pp in DbUtil.CurrentDatabase.People
            ...
            if (isvalid)
            {
                statusflags = string.Join(",", from s in DbUtil.CurrentDatabase.StatusFlagsPerson(id).ToList()
        ...
        public string CheckView()
        {
            ...
            {
                var olotag = DbUtil.CurrentDatabase.OrgLeadersOnlyTag2();
                if (!DbUtil.CurrentDatabase.TagPeople.Any(pt => pt.PeopleId == PeopleId && pt.Id == olotag.Id))
                {
        ...
        public List<ResourceTypeModel> ResourceTypes
        {
            get
            {
                ...
                var memberOrgs = DbUtil.CurrentDatabase.OrganizationMembers.Where(x => x.PeopleId == PeopleId).ToList();
                ...
                var divisionIds = DbUtil.CurrentDatabase.OrganizationMembers.Where(x => x.PeopleId == PeopleId)
                    .Select(x => x.Organization.DivisionId).Distinct().ToList();

                var orgTypeIds = DbUtil.CurrentDatabase.Organizations
                ...
                // same for campus
                var resources =
                    DbUtil.CurrentDatabase.Resources
        ...
        private bool ShouldShowResource(Resource resource)
        {
            ...
            var personStatusFlags = DbUtil.CurrentDatabase.ViewAllStatusFlags.Where(sf => sf.PeopleId == PeopleId).Select(x => x.Flag).ToList();
