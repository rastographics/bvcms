...
namespace CmsWeb.Areas.People.Models.Task
{
	public class TaskModel
	{
		...
		public static bool CompleteTask( User user, int id )
		{
			var task = DbUtil.Db.Tasks.SingleOrDefault( t => t.Id == id );
			...
			ChangeTask( sb, task, "StatusId", TaskStatusCode.Complete );

			NotifyIfNeeded( user, task, sb );

			DbUtil.Db.SubmitChanges();

			if( task.Owner.PeopleId == user.PeopleId ) {
				if( task.CoOwner != null ) {
					GCMHelper.sendNotification( task.CoOwner.PeopleId, GCMHelper.TYPE_TASK, task.Id, "Task Complete", $"{user.Person.Name} completed a task they delegated to you" );
				}
			} else {
				GCMHelper.sendNotification( task.Owner.PeopleId, GCMHelper.TYPE_TASK, task.Id, "Task Complete", $"{user.Person.Name} completed a task you delegated them" );
			}
		...
		public static bool DeclineTask( User user, int id, string reason )
		{
			var task = DbUtil.Db.Tasks.SingleOrDefault( t => t.Id == id );
			...
			DbUtil.Db.SubmitChanges();
			DbUtil.Db.Email( task.CoOwner.EmailAddress, task.Owner, $"Task declined by {user.Person.Name}", CreateEmailBody( task ) );

			GCMHelper.sendNotification( task.Owner.PeopleId, GCMHelper.TYPE_TASK, task.Id, "Task Declined", $"{user.Person.Name} declined a task" );
		...
		public static void NotifyIfNeeded( User user, CmsData.Task task, StringBuilder sb )
		{
			...
			DbUtil.Db.Email( from.EmailAddress, to, $"Task updated by {user.Person.Name}", CreateEmailBody( task ) );
		}
