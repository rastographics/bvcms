...
CREATE FUNCTION [dbo].[PastAttendanceCredits](@orgid INT, @pid INT) 
RETURNS @t TABLE ( 
	Attended BIT, 
	[Year] INT, 
	[Week] INT, 
...
BEGIN 
    ...
    SELECT @lastmeet = MAX(MeetingDate) FROM dbo.Attend WHERE PeopleId = @pid AND OrganizationId = @orgid 
