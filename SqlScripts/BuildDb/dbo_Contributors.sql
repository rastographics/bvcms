CREATE FUNCTION [dbo].[Contributors](@fd DATETIME, @td DATETIME, @pid INT, @spid INT, @fid INT, @noaddrok BIT, @tagid INT)
RETURNS TABLE 
AS
RETURN 
(
	SELECT	
		    ...
		    ISNULL((SELECT SUM(ContributionAmount)
				...
				AND (ISNULL(f.NonTaxDeductible, 0) = 0 OR c.ContributionTypeId = 9)
