ALTER FUNCTION [dbo].[ContributionSearch]
(	
	@Name VARCHAR(100)
...
BEGIN
	...
	INSERT @funds (FundId) SELECT Value FROM dbo.SplitFundIds(@fundids)
	...
	AND CASE ISNULL(@TaxNonTax, 'TaxDed')
		   WHEN 'TaxDed' THEN IIF(@IncludePledges = 1, IIF(c.ContributionTypeId NOT IN (9), 1, 0), IIF(c.ContributionTypeId NOT IN (9, 8), 1, 0))
		   WHEN 'NonTaxDed' THEN IIF(@IncludePledges = 1, IIF(c.ContributionTypeId IN (9, 8), 1, 0),IIF(c.ContributionTypeId = 9, 1, 0))
		   WHEN 'Both' THEN IIF(@IncludePledges = 1, 1,IIF(c.ContributionTypeId <> 8, 1, 0))
