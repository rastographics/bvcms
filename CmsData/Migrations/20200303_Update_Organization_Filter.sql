...
(
    ...
    -- TPD-447: At this point, NameSplit has *already* trimmed all names of trailing whitespace.
	WHERE
    (
        p.FirstName LIKE
        ...
        END
        AND p.LastName LIKE
        CASE WHEN f.LastName IS NULL THEN -- if LastName is null, assume that we are either searching by first name or doing a fuzzy search.
            '%'
        ELSE -- LastName is *not* null. At this point, we might have [FirstName, LastName] OR [NULL, LastName]
            ('%' + f.LastName + '%')
        END
    ) -- if this fails, we can assume that FirstName is null; ergo, the next block will catch it.
    OR p.LastName LIKE
    CASE WHEN f.LastName IS NULL THEN
        ('%' + f.FirstName + '%')
    ELSE
