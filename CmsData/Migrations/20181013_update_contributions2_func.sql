...
SELECT 
    ...
    CASE WHEN fa.HeadOfHouseholdId = sp.PeopleId
			...
			AND ISNULL(p.ContributionOptionsId, CASE WHEN mssp.Married = 1 THEN 2 ELSE 1 END) = 2
		THEN sp.PeopleId 
		ELSE c.PeopleId 
	END AS CreditGiverId,

    CASE WHEN ISNULL(sp.ContributionOptionsId, CASE WHEN mssp.Married = 1 THEN 2 ELSE 1 END) = 1
			OR ISNULL(p.ContributionOptionsId, CASE WHEN mssp.Married = 1 THEN 2 ELSE 1 END) = 1
		THEN NULL
