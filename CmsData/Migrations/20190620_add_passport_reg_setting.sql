GO
ALTER VIEW dbo.RegsettingOptions
AS
SELECT OrganizationId, OrganizationName,
      ...
      AskPassport = CAST(IIF(RegSettingXml.value('(/Settings/AskItems/AskPassport)[1]', 'varchar(10)') IS NOT NULL, 1, NULL) AS BIT),
...
IF NOT EXISTS(SELECT 1 FROM sys.columns
          WHERE Name = N'passportnumber'
          AND Object_ID = Object_ID(N'dbo.RecReg'))
BEGIN
    ALTER TABLE dbo.RecReg
    ADD passportnumber NVARCHAR(100) NULL
END
GO
IF NOT EXISTS(SELECT 1 FROM sys.columns
          WHERE Name = N'passportexpires'
          AND Object_ID = Object_ID(N'dbo.RecReg'))
BEGIN
    ALTER TABLE dbo.RecReg
    ADD passportexpires NVARCHAR(100) NULL
END
GO
