ALTER FUNCTION [dbo].[FilterOrgSearchName](@name VARCHAR(100))
RETURNS 
@t TABLE ( oid INT )
AS
BEGIN
	...
	BEGIN
		INSERT @t SELECT dbo.Organizations.OrganizationId FROM dbo.Organizations WHERE dbo.Organizations.OrganizationName LIKE '%' + @name + '%'
		INSERT @t SELECT dbo.Organizations.OrganizationId FROM dbo.Organizations WHERE dbo.Organizations.OrganizationId = @name
		RETURN
	END
