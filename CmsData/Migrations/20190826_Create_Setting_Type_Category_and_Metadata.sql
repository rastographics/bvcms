...
('GoogleGeocodeAPIKey', 'Copy the value from the key named touchpoint-gmap and paste it here. See Small Group Finder - Embedding a Map. https://docs.touchpointsoftware.com/Organizations/SmallGroupFinderMap.html', 3, 2, 10),
('GoogleMapsAPIKey', 'Copy the value from the key named touchpoint-geocode. See Small Group Finder - Embedding a Map. https://docs.touchpointsoftware.com/Organizations/SmallGroupFinderMap.html', 3, 2, 10),
...
('RelaxAppAddGuest', 'This setting with a value of true allows an OrgLeaderOnly user to access all records in the database, but only when using the iPhone/Android app to add a guest when taking attendance. See also iPhone / Android Attendance with App. https://docs.touchpointsoftware.com/Organizations/RecordingAttendanceiPhone.html', 1, 2, 7),
('RelaxedReqAdminOnly', 'Add this Setting and set it to true if you want only an Admin user to be able to relax requirements for an Online Registration. With this Setting, other users do not even see those options on the Registration tab.', 1, 2, 8),
...
('Resources-Enabled', 'Enter a value of true to enable the Resources feature on your database. See also Resources', 1, 2, 9),
('SGF-LoadAllExtraValues', 'Loads all extra values, even if they lack the SGF prefix', 1, 2, 10),
('SGF-OrgTypes', 'To include only organizations of a specified Organization Type in the group finder, enter a value of the desire Organization Type. For example, to include only groups of the type Community Groups, give the setting the value Community Groups.', 3, 2, 10),
('SGF-UseEmbeddedMap', 'Set this to true to embed a map in your Small Group Finder. See Small Group Finder - Embedding a Map', 1, 2, 10),
('SendLinkExpireMinutes', 'Set this to the number of minutes after which a SendLink expires. The default is 30.', 3, 2, 8),
...
('UX-MapPinTextColor', 'Use to set the text color within the SGF map pin (hexadecimal RGB)', 3, 2, 10),
('UX-SGFSortBy', 'Use this setting to specify a particular extra value field to sort the groups in the finder. For example, if you give this setting the value SGF:Neighborhood, the groups will be sorted alphabetically by the values in the SGF:Neighborhood extra value.', 3, 2, 10),
...
('EnableBackgroundChecks', 'This setting must be in the database and set to true in order to use the Protect My Ministry integration.', 1, 3, 11),
('EnableBackgroundLabels', 'Add this setting and and enter true as the Value if you want to use labels from your Protect My Ministry account. You must also add these labels to the Lookups. See also Background Checks with Protect My Ministry. https://docs.touchpointsoftware.com/People/ProtectMyMinistry-BackgroundCh.html', 1, 3, 11),
('PMMPassword', 'By default, this is empty. If you use the integration with Protect My Ministry for background checks, enter your account password as the value. See also Background Check with Protect My Ministry. https://docs.touchpointsoftware.com/People/ProtectMyMinistry-BackgroundCh.html', 4, 3, 11),
...
('TwilioSID', 'If you are using the integration with Twilio for SMS (text) messaging, you will get this ID# from your Twilio account. Enter it as the value. See also Establish an Account with Twilio', 3, 3, 14),
