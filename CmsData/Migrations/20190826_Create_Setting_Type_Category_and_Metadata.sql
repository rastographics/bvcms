...
('BankDepositFormat', 'This is needed only for those churches using Vanco for online giving. Enter <i>vanco</i> as the Setting value.', 3, 1, 3),
('CampusIds', 'If you are using the <b>ShowCampusOnRegistration</b> Setting, you must also use this Setting to indicate which Campuses you want to offer as options for a new record. This applies only to the drop down that displays when a registrant creates a new record during an online registration. Enter the Campus ID #s, separated by a comma, without any spaces. You can enter all the Campuses or only a select few. See the Setting regarding <b>ShowCampusOnRegistration</b> for more information.', 3, 2, 9),
('CheckImagesDisabledForUser', 'Set this to <i>true</i> to hide the check images on a person`s giving tab.', 1, 1, 3),
('ContributionStatementFundDisplayFieldName', 'If you want the Fund Descriptions rather than Fund Names to display on contribution statements, enter the value of <i>FundDescription</i>. To revert to the default, enter the value of <i>FundName</i>.', 3, 1, 3),
('CustomBundleReport', 'Enter the appropriate value from the <a href="https://docs.touchpointsoftware.com/CustomProgramming/Python/Scripts/BundleReport.html" target="_blank">Bundle Report</a> or <a href="https://docs.touchpointsoftware.com/CustomProgramming/Python/Scripts/BundleReport2.html" target="_blank">Bundle Report 2</a> SQL recipes.', 3, 1, 3),
...
('DebitCreditLabel-Giving', 'This Setting, similar to <b>DebitCreditLabel</b>, will allow you to define the label displayed, but only on the payment pages for giving transactions.', 3, 1, 3),
('DebitCreditLabel-Registrations', 'This Setting, similar to <b>DebitCreditLabel</b>, will allow you to define the label displayed, but only on the payment pages for registration payments.', 3, 1, 3),
...
('DefaultCampusFunds-x', 'Use this Setting to specify the funds to display on the managed giving page for the specified campus. In the setting, the -x at the end should be replaced with the campus ID. If, for example, your Main campus has ID 1, enter the setting DefaultCampusFunds-1 and, as its value, enter the IDs of the funds you wish to be displayed for that campus. If you enter multiple fund IDs, separate them with commas. Use DefaultCampusFunds-0 to specify funds to display if you do not have multiple campuses, or if a donor has no campus specified on their record.', 3, 1, 3),
('DefaultFundId', 'Use this Setting if you want a fund other than Fund ID <i>1</i> to be the default fund. When creating a new Bundle, this is the fund that always appears at the top of the list of funds when setting the default for the bundle.', 3, 1, 3),
...
('Feature-ContactExtra', 'Set this value to <i>true</i> if you want to use the feature that allows you to add Extra Values to a Contact form. See also <a href="https://docs.touchpointsoftware.com/ContactsAndTasks/AddExtraValuesToContact.html" target="_blank">Contact Extra Values</a>.', 1, 2, 6),
('GoogleGeocodeAPIKey', 'Copy the value from the key named <i>touchpoint-gmap</i> and paste it here. See <a href="https://docs.touchpointsoftware.com/Organizations/SmallGroupFinderMap.html" target="_blank">Small Group Finder - Embedding a Map</a>.', 3, 2, 10),
('GoogleMapsAPIKey', 'Copy the value from the key named <i>touchpoint-geocode</i>. See <a href="https://docs.touchpointsoftware.com/Organizations/SmallGroupFinderMap.html" target="_blank">Small Group Finder - Embedding a Map</a>.', 3, 2, 10),
('LimitRegistrationHistoryToRole', 'Enter the role name required to view the registration history for others.', 3, 2, 8),
('LimitToRolesForContacts', 'This Setting will let you determine which user roles will display in the <i>Limit To Role</i> drop down list on the Contact form when a user is recording a Contact. Enter the user roles (comma separated) in the <i>Value</i> field that you want to display in the drop down. See also <a href="https://docs.touchpointsoftware.com/ContactsAndTasks/AddContact.html" target="_blank">Add a Contact</a>.', 3, 2, 6),
('MinContributionAmount', 'Enter the minimum dollar amount required for a contribution statement to generate. For example: 5 or 200. Churches that send statements every quarter may set this to a higher amount for the first 3 quarters in order not to send statements to children. Then, they may change it to a lower amount for the yearly statement in order to include everyone.', 3, 1, 3),
('NoCreditCardGiving', 'By default, this is set to <i>false</i>, meaning that donors can use a credit card for online giving. Set the value to <i>true</i> if you want donors to only use their bank account for giving. <b>Note:</b> Even with this Setting, online giving for Mission Trip Support and Ask Extra Donations will allow credit card use.', 1, 1, 3),
('NoEChecksAllowed', 'Set the value to <i>true</i> if you want people to only use credit cards for giving and for online registration payments.', 1, 1, 3),
('NoPrintDateOnStatement', 'The default is to display (in the top-right corner of the statement) the date it was generated (print date). If you do not want this to display, set the value to <i>true</i>.', 1, 1, 3),
('NoTitlesOnStatements', 'Set the value to <i>true</i> if you want the names of joint statements to read as follows: first and last name of the head of household, followed by the first and last name of the spouse. Examples: John Smith and Mary Smith; or Frank Jones and Janet Williams-Jones.', 1, 1, 3),
...
('RequireAddressOnStatement', 'Set the value to <i>false</i> if you want contribution statements to print (when printing All Statements), even if the record has no address. By default, if there is no address, a statement will not generate, except from the individualâ€™s record.', 1, 1, 3),
('RequireCheckNoOnStatement', 'If you want check numbers to print on Contribution Statements, set the value to <i>true</i>. If you print check numbers, the statements will print only one column on the page, instead of wrapping to make two columns. You can choose to print both the check numbers and the notes (see below). See also <a href="https://docs.touchpointsoftware.com/Finance/PostBundle_Index.html" target="_blank">Post Bundle - Record Individual Contributions</a>.', 1, 1, 3),
...
('Resources-Enabled', 'Set the value to <i>true</i> to enable the Resources feature on your database. See also <a href="https://docs.touchpointsoftware.com/People/Resources.html" target="_blank">Resources</a>.', 1, 2, 9),
...
('SGF-UseEmbeddedMap', 'Set this to true to embed a map in your Small Group Finder. See <a href="https://docs.touchpointsoftware.com/Organizations/SmallGroupFinderMap.html" target="_blank">Small Group Finder - Embedding a Map</a>', 1, 2, 10),
('SendLinkExpireMinutes', 'Set this to the number of minutes after which a <a href="https://docs.touchpointsoftware.com/EmailTexting/ComposeSendLinks.html" target="_blank">SendLink</a> expires. The default is 30.', 3, 2, 8),
('SendRecurringGiftFailureNoticesToFinanceUsers', 'By default, this is set to <i>false</i> and these notices are sent only to the Online Notify Person(s) on the Online Giving Organizations. If you prefer for all users with <b>Finance</b> role to receive these, change the value to <i>true</i>.', 1, 1, 3),
...
('TwilioSID', 'If you are using the integration with Twilio for SMS (text) messaging, you will get this ID# from your Twilio account. Enter it as the value. See also <a href="https://docs.touchpointsoftware.com/EmailTexting/TextingTwilio.html" target="_blank">Establish an Account with Twilio</a>.', 3, 3, 14),
...
('AdminMail', 'This is the email address that will be used for any Admin notifications, and from whom standard automated notifications will be sent. See also <a href="https://docs.touchpointsoftware.com/GettingStarted/FirstSteps.html" target="_blank">First Steps with your TouchPoint Database</a>.', 3, 1, 1),
...
('ElectronicStatementDefault', 'Set this to <i>true</i> to have this flag set automatically for all new records added to the database. See also <a href="https://docs.touchpointsoftware.com/Finance/ElectronicOnlyStatements.html" target="_blank">Electronic Only Statement Option</a>.', 1, 1, 3),
('EmailPromptBeforeSend', 'Set this to <i>true</i> to add a prompt that will appear after you click <i>Send</i>, asking â€œAre You Sure?â€ and giving you an opportunity to cancel or continue.', 1, 1, 1),
('EnableContributionFundsOnStatementDisplay', 'To make custom statements available electronically, so that donors can download them from their profiles, set this to <i>true</i>. See also <a href="https://docs.touchpointsoftware.com/Finance/CustomStatements.html" target="_blank">Custom Contribution Statements</a>.', 1, 1, 3),
('EnforceEditCampusRole', 'Set this to <i>true</i> when you want only those with the user role <b>EditCampus</b> to be able to change/edit the Campus on a personâ€™s record. This applies only to the Campus field on a personâ€™s record, not to setting the Campus for an organization. After adding this Setting, add the User Role <b>EditCampus</b> in the Lookup Codes and assign that role to everyone that should be able to edit the Campus for an individual. Exception: If you have this Setting, a My Data user (and any other user) can edit his and his familyâ€™s Campus without having the <b>EditCampus</b> role if you have the Setting <b>MyDataCanEditCampus</b>.', 1, 1, 1),
...
('ExternalManageGivingUrl', 'If your donors give through an external giving service rather than through TouchPoint, enter the URL to your recurring giving site. This will configure the link for managed giving on a donorâ€™s <i>Giving</i> tab to point to your external giving service.', 3, 1, 3),
...
('MorningBatchRan', 'If you are using Morning Batch, this will get created automatically and display the last date/time that it ran. See also <a href="https://docs.touchpointsoftware.com/Administration/MorningBatch.html" target="_blank">Morning Batch</a>.', 2, 1, 2),
...
('PictureDirectorySelector', 'Enter the name of the Status Flag search you are using for the Online Picture Directory. See also <a href="https://docs.touchpointsoftware.com/SummaryReports/OnlinePictureDirectory/QuickStart.html" target="_blank">Online Picture Directory</a>.', 3, 1, 1),
...
('StatusFlags', 'By default, the StatusFlags F01,F02,and F03 are in new databases, but each church can configure these to whatever they want. The order that you enter them in this Setting is the order in which they will display on a personâ€™s record. See also <a href="https://docs.touchpointsoftware.com/People/StatusFlags.html" target="_blank">Status Flags</a>.', 3, 1, 1),
('SubjectAttendanceNotices', 'Enter the text you want as the subject line of the Email Attendance Notices, if you do not want to use the default subject. See also <a href="https://docs.touchpointsoftware.com/Organizations/EmailAttendanceNotices.html" target="_blank">Email Attendance Notices</a>.', 3, 1, 1),
