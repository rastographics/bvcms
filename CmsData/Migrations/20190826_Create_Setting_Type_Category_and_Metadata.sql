...
('BankDepositFormat', 'This is needed only for those churches using Vanco for online giving. Enter <i>vanco</i> as the Setting value.', 3, 2, 3),
...
('CheckImagesDisabledForUser', 'Set this to <i>true</i> to hide the check images on a person`s giving tab.', 1, 2, 3),
...
('DefaultFundId', 'Use this Setting if you want a fund other than Fund ID <i>1</i> to be the default fund. When creating a new Bundle, this is the fund that always appears at the top of the list of funds when setting the default for the bundle.', 5, 2, 3),
...
('MinContributionAmount', 'Enter the minimum dollar amount required for a contribution statement to generate. For example: 5 or 200. Churches that send statements every quarter may set this to a higher amount for the first 3 quarters in order not to send statements to children. Then, they may change it to a lower amount for the yearly statement in order to include everyone.', 5, 2, 3),
...
('RequireAddressOnStatement', 'Set the value to <i>false</i> if you want contribution statements to print (when printing All Statements), even if the record has no address. By default, if there is no address, a statement will not generate, except from the individualâ€™s record.', 1, 2, 3),
('RequireCheckNoOnStatement', 'If you want check numbers to print on Contribution Statements, set the value to <i>true</i>. If you print check numbers, the statements will print only one column on the page, instead of wrapping to make two columns. You can choose to print both the check numbers and the notes (see below). See also <a href="https://docs.touchpointsoftware.com/Finance/PostBundle_Index.html" target="_blank">Post Bundle - Record Individual Contributions</a>.', 1, 2, 3),
...
('DefaultCampusId', 'Enter the ID# (not the name) of the Campus to use as the default campus on new records and new organizations. Leave empty if your church does not have more than one campus in the database.', 5, 1, 2),
('DefaultHost', 'This is the URL of your database and will be set for you.', 3, 1, 1),
('DisplayNonTaxOnStatement', 'Set this to <i>true</i> if you want all non tax deductible contributions to print on contribution statements. These print in a separate section from the other contributions.', 1, 2, 3),
('ElectronicStatementDefault', 'Set this to <i>true</i> to have this flag set automatically for all new records added to the database. See also <a href="https://docs.touchpointsoftware.com/Finance/ElectronicOnlyStatements.html" target="_blank">Electronic Only Statement Option</a>.', 1, 2, 3),
...
('EnableContributionFundsOnStatementDisplay', 'To make custom statements available electronically, so that donors can download them from their profiles, set this to <i>true</i>. See also <a href="https://docs.touchpointsoftware.com/Finance/CustomStatements.html" target="_blank">Custom Contribution Statements</a>.', 1, 2, 3),
...
('ExternalManageGivingUrl', 'If your donors give through an external giving service rather than through TouchPoint, enter the URL to your recurring giving site. This will configure the link for managed giving on a donorâ€™s <i>Giving</i> tab to point to your external giving service.', 3, 2, 3),
...
('MaxExcelRows', 'By default, this is set to 10,000 for Excel exports.', 5, 1, 1),
...
('MinimumUserAge', 'This Setting is an age, such as <i>13</i>. The default is <i>16</i>. This applies to people trying to create a user account on the church database. For churches that do not capture dates of birth, they may want to make the value <i>0</i>. This will allow a person without a date of birth to create a user account.', 5, 1, 4),
('MonthFirst', 'For dates, this will display the month, then the day. By default, it is set to <i>true</i>.', 1, 1, 1),
('MorningBatchRan', 'If you are using Morning Batch, this will get created automatically and display the last date/time that it ran. See also <a href="https://docs.touchpointsoftware.com/Administration/MorningBatch.html" target="_blank">Morning Batch</a>.', 2, 1, 1),
...
('PasswordMinLength', 'Use a number, such as <i>7</i>.', 5, 1, 4),
...
('TZOffset', 'Enter the number of hours (plus or minus) offset from the Central Time zone. If you are in the Central Time zone, you can leave this blank. If, for example, you are in the Eastern Time zone, enter <i>1</i> for the value.', 3, 2, 5),
