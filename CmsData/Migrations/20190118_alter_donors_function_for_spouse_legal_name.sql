ALTER FUNCTION [dbo].[Donors](@fd DATETIME, @td DATETIME, @pid INT, @spid INT, @fid INT, @noaddrok BIT, @tagid INT, @funds VARCHAR(MAX))
RETURNS TABLE 
AS
RETURN 
(
	SELECT	
			...
			ISNULL(sp.FirstName, '') + ' ' + ISNULL(sp.LastName, '') AS SpouseNameWithoutNickname,
