...
namespace CmsData.Finance
{
    public class BluePayGateway : IGateway
    {
        ...
        {
            this.db = db;
            IsLive = !(testing || MultipleGatewayUtils.GatewayTesting(ProcessType));

            _login = MultipleGatewayUtils.Setting("bluepay_accountId", "", (int)ProcessType);
            _key = MultipleGatewayUtils.Setting("bluepay_secretKey", "", (int)ProcessType);
