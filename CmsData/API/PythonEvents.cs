...
namespace CmsData
{
    ...
    public class PythonEvents : IPythonApi
    {
        ...
        public static void RegisterHelpers(CMSDataContext db)
        {
            ...
            /// IfInOrg in the form of:  {{IfInOrg pid orgId}} {{/IfInOrg}}
            Handlebars.RegisterHelper("IfInOrg", (writer, options, context, args) =>
            {
                var pid = args[0];
                var orgId = args[1];
            ...
            /// IfInSubGroup in the form of:  {{IfInSubGroup pid orgId "subgroup"}} {{/IfInSubGroup}}
            /// subgroup is case sensitive
            Handlebars.RegisterHelper("IfInSubGroup", (writer, options, context, args) =>
            {
                var pid = args[0];
                var orgId = args[1];
                ...
                {
                    writer.Write("PID not in Org");
                }
            });

            /// Math helper in form of:  {{Math lval oper rval "fmt?"}}
            /// ex. {{Math Total "/" 12}}
            ...
            Handlebars.RegisterHelper("Math", (writer, context, args) =>
            {
                ...
                try { double.TryParse(args[0].ToString(), out lval); }
                catch { err = "lval not a number"; }

                try { double.TryParse(args[2].ToString(), out rval); }
                ...
                switch (oper.ToString())
                {
                    ...
                    case "%":
                        ...
                        break;
                    default:
                        err = "invalid math oper";
                        break;
                 }

                if (err.Length == 0)
                {
                    if (args.Length == 3) fmt = "{0:G}";
                    else fmt = "{0:" + args[3].ToString() + "}";
                    writer.Write(String.Format(fmt, result));
                }
                else writer.Write(err);
            });

            /// Format Number helper in form of:  {{FmtNum value "fmt"}}
            /// ex. {{FmtNum Total "C"}}
            /// fmt is required. Uses standard/custom dotnet format strings
            Handlebars.RegisterHelper("FmtNum", (writer, context, args) =>
            {

                Double val = Convert.ToDouble(args[0]);
                string fmt = "{0:" + args[1].ToString() + "}";
                writer.Write(String.Format(fmt, val));
            });

            /// FmtAddress in the form of:  {{FmtAddress PID "type" lines}}
            /// type: "F" for family or "P" for Personal or "Primary" for primary
            /// Lines: 1, 2 or 3  - format address with <BR> codes to fit on 1, 2 or 3 lines
            Handlebars.RegisterHelper("FmtAddress", (writer, context, args) =>
            {
                var pid = args[0].ToInt();
                var type = args[1].ToString();
                var lines = args[2].ToInt();
                var line1 = "";
                var line2 = "";
                var city = "";
                var state = "";
                ...
                try
                {
                    p = db.LoadPersonById(pid.ToInt());
                    f = db.LoadFamilyByPersonId(pid.ToInt());
                }
                catch
                {
                    line1 = "Could Not Load Person";
                }
                ...
                switch (type)
                {
                    case "P":
                        if (p != null)
                        {
                            ...
                            line1 = p.AddressLineOne;
                            line2 = p.AddressLineTwo;
                            city = p.CityName;
                            state = p.StateCode;
                            if (p.ZipCode.IsNotNull()) zip = p.ZipCode.ToString().FmtZip();
                        }
                        break;
                    case "Primary":
                        if (p != null)
                        {
                            ...
                            line1 = p.PrimaryAddress;
                            line2 = p.PrimaryAddress2;
                            city = p.PrimaryCity;
                            state = p.PrimaryState;
                            if (p.PrimaryZip.IsNotNull()) zip = p.PrimaryZip.ToString().FmtZip();
                        }
                        break;
                    case "F":
                    default:
                        if (f != null)
                        {
                            ...
                            city = f.CityName;
                            state = f.StateCode;
                            if (f.ZipCode.IsNotNull()) zip = f.ZipCode.ToString().FmtZip();
                        }
                        break;
                }
                ...
                switch (lines)
                {
                    case 1:
                        ...
                        if (line2.IsNull())
                            outputstr = line1 + " " + city + ", " + state + " " + zip;
                        else
                            outputstr = line1 + " " + line2 + ",  " + city + ", " + state + " " + zip;
                        break;
                    case 2:
                        if (line1.IsNull()) break;
                        if (line2.IsNull())
                            outputstr = line1 + "</BR>" + city + ", " + state + " " + zip;
                        else
                            outputstr = line1 + " " + line2 + "</BR>" + city + ", " + state + " " + zip;
                        break;
                    case 3:
                    default:
                        if (line1.IsNull()) break;
                        if (line2.IsNull())
                            ...
                            outputstr = line1 + "</BR>" + line2 + "</BR>" + city + ", " + state + " " + zip;
                        break;
                }

                writer.WriteSafeString(outputstr);

            });

            /// Now in the form of:  {{Now "fmt?"}}
            /// Standard output format is M/d/YYYY
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            Handlebars.RegisterHelper("Now", (writer, context, args) =>
            {
                DateTime now = DateTime.Now;
                string fmt;
                if (args.Length == 0) fmt = "{0:M/d/yyyy}";
                else fmt = "{0:" + args[0].ToString() + "}";
                writer.Write(String.Format(fmt, now));

            });


            /// FmtDate in the form of:  {{FmtDate Date "fmt?"}}
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            /// default format is M/d/yyy
            Handlebars.RegisterHelper("FmtDate", (writer, context, args) =>
            {
                DateTime dt;
                try { dt = Convert.ToDateTime(args[0].ToString()); }
                catch
                {
                    dt = DateTime.MinValue;
                    writer.WriteSafeString("Invalid Date: " + args[0].ToString() + "</BR>");
                }
                
                string fmt;
                if (args.Length == 1) fmt = "{0:M/d/yyyy}";
                else fmt = "{0:" + args[1].ToString() + "}";
                writer.Write(String.Format(fmt, dt));

            });

            /// WeekNumber in the form of:  {{WeekNumber Date}}
            Handlebars.RegisterHelper("WeekNumber", (writer, context, args) =>
            {
                DateTime dt;

                if (args[0].ToString().ToLower() == "now") dt = DateTime.Now;
                else
                {
                    try { dt = Convert.ToDateTime(args[0]); }
                    catch
                    {
                        dt = DateTime.MinValue;
                        writer.WriteSafeString("Invalid Date: " + args[0].ToString() + "</BR>");
                ...
                {
                    var val = dt.GetWeekNumber();
                    writer.Write(val.ToString());
                }
                else
                {
                    writer.Write("bad date: " + args[0].ToString());
                }              

            });

            /// SundayForDate in the form of:  {{SundayForDate Date "fmt?"}}
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            /// default format is M/d/yyy
            Handlebars.RegisterHelper("SundayForDate", (writer, context, args) =>
            {
                DateTime dt;
                string fmt;
            ...
            /// SundayForWeek in the form of:  {{SundayForWeek year week "fmt?"}}
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            /// default format is M/d/yyy
            Handlebars.RegisterHelper("SundayForWeek", (writer, context, args) =>
            {
                DateTime sun = Util.SundayForWeek(args[0].ToInt(), args[1].ToInt());
                string fmt;
                if (args.Length <= 2) fmt = "{0:M/d/yyyy}";
                else fmt = "{0:" + args[2].ToString() + "}";
                writer.Write(String.Format(fmt, sun));
             });

            /// MostRecentAttendedSunday in the form of:  {{MostRecentAttendedSunday progid "fmt?"}}
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            /// default format is M/d/yyy
            Handlebars.RegisterHelper("MostRecentAttendedSunday", (writer, context, args) =>
            {
                int progid = args[0].ToInt();
            ...
            /// DateOffset in the form of:  {{DateOffset date offset "type" "fmt?"}}
            /// fmt is optional. Uses standard/custom dotnet DateTime format strings
            /// default format is M/d/yyy
            Handlebars.RegisterHelper("DateOffset", (writer, context, args) =>
            {
                DateTime dt;
                ...
                string fmt;
                double offset = Convert.ToDouble(args[1]);
                string type = args[2].ToString();
                
                switch (type)
                {
                    case "Y":
                        dt2 = dt.AddYears(offset.ToInt());
                        break;
                    case "M":
                        dt2 = dt.AddMonths(offset.ToInt());
                        break;
                    case "D":
                    default:
                        dt2 = dt.AddDays(offset);
                        break;
                }

                if (args.Length <= 3) fmt = "{0:M/d/yyyy}";
                else fmt = "{0:" + args[3].ToString() + "}";
                writer.Write(String.Format(fmt, dt2));
            ...
            {
                string phonenum = args[0].ToString();
